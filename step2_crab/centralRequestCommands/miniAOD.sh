...
# Dump actual test code to a HIG-Run3Summer22MiniAODv4-01559_test.sh file that can be run in Singularity
cat <<'EndOfTestFile' > HIG-Run3Summer22MiniAODv4-01559_test.sh
#!/bin/bash
...
# cmsDriver command
cmsDriver.py  --era Run3,run3_miniAOD_12X --customise Configuration/DataProcessing/Utils.addMonitoring --step PAT --geometry DB:Extended --conditions 130X_mcRun3_2022_realistic_v5 --datatier MINIAODSIM --eventcontent MINIAODSIM --python_filename HIG-Run3Summer22MiniAODv4-01559_1_cfg.py --fileout file:HIG-Run3Summer22MiniAODv4-01559.root --filein file:HIG-Run3Summer22DRPremix-01527.root --number 600 --number_out 600 --no_exec --mc || exit $? ;

# Run generated config
REPORT_NAME=HIG-Run3Summer22MiniAODv4-01559_report.xml
# Run the cmsRun
cmsRun -e -j $REPORT_NAME HIG-Run3Summer22MiniAODv4-01559_1_cfg.py || exit $? ;

# Parse values from HIG-Run3Summer22MiniAODv4-01559_report.xml report
...
echo "Validation report of HIG-Run3Summer22MiniAODv4-01559 sequence 1/1"
...
# End of HIG-Run3Summer22MiniAODv4-01559_test.sh file
EndOfTestFile

# Make file executable
chmod +x HIG-Run3Summer22MiniAODv4-01559_test.sh
...
singularity run --home $PWD:$PWD /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/$CONTAINER_NAME $(echo $(pwd)/HIG-Run3Summer22MiniAODv4-01559_test.sh)
